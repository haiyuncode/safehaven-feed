...
const sources: Sources = JSON.parse(
  fs.readFileSync("data/sources.json", "utf-8")
);
...
async function buildTopic(topic: Topic) {
  ...
  const videos = (
    await Promise.all(
      src.youtubeChannels.map((c) => fetchYouTubeChannel(c.channelId))
    )
  ).flat();
  const articles = (
    await Promise.all(src.blogs.map((b) => fetchBlog(b.rss)))
  ).flat();
